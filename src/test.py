...
class TestDao(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.instrument_types = self._raw_queryset_with_id_to_set(self.dao.get_instrument_types())
    ...
    def test_instruments(self):
        ...
        for iid, instrument in self.dao.get_instrument_with_xccy_rate(epoch2date(TODAY)).items():
    ...
    def test_funds(self):
        for pos in self.dao.get_funds_positions(TODAY):
            ...
            self.assertEqual(self.instruments[pos['instrument_id']].instrument_type.name, 'Funds')
    ...
    def test_stocks(self):
        stock = get_random_dict_value(self.instruments,
        ...
        self.assertTrue(self.dao.update_stock_transaction(stock.name, {"Price": price, "Type": trans_type,
                                                                       "Date": dt, "Fee": fee,
                                                                       "Shares": shares}))
...
def test_update_instrument(self):
    instrument = get_random_dict_value(self.instruments,
                                       lambda x: True if x.instrument_type.name == 'Funds' else False)
    ...
    a1 = assets.pop()
    a2 = assets.pop()
    self.assertTrue(self.dao.update_instrument_asset_allocations(instrument.name, {
        'assets': [{'asset': a1, 'ratio': 20}, {'asset': a2, 'ratio': 80}]}))
    a = {t: r for t, r in self.dao.get_asset_allocation(instrument_name=instrument.name)}
    ...
    c1 = countries.pop()
    c2 = countries.pop()
    self.assertTrue(self.dao.update_instrument_country_allocations(instrument.name, {
        'countries': [{'country': c1, 'ratio': 20}, {'country': c2, 'ratio': 80}]}))
    c = {t: r for t, r in self.dao.get_country_allocation(instrument_name=instrument.name)}
