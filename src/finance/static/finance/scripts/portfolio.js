...
function get_active_portfolio() {
  var name = $("#portfolios")
    .find(".btn-success")
    .attr("id")
    .slice(10); //remove the leading portfolio_prefix
  return name;
}
  ...
  $.getJSON(portfolio_url, function(portfolios) {
    ...
    $.each(portfolios, function(i, v) {
      ...
      $.each(v["allocations"], function(i2, alloc) {
        ...
        alloc['original_shares'] = alloc['shares'];
        alloc['original_market_value'] = alloc['market_value'];
        alloc['original_current_allocation'] = alloc['current_allocation'];
...
function rebalance() {
  $(document).ready(function() {
    function populate_plan_data(id, new_fund, plan_data) {
      ...
      $(id.concat("_new_funds")).text(
        "New funds: ".concat(format_num(new_fund))
      );
    ...
    const new_fund = 1000;
    var url = "/finance/api/report/rebalance_portfolio/".concat(
      portfolio_name,
      "/",
      new_fund
    );
