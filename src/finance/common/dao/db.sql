...
CREATE TABLE instrument (id INTEGER PRIMARY KEY, name text not null, type int null, broker null, currency int null, "url" TEXT, "expense_ratio" FLOAT, FOREIGN KEY(currency) REFERENCES currency(id),FOREIGN KEY(type) REFERENCES instrument_type(id),FOREIGN KEY(broker) REFERENCES broker(id));
